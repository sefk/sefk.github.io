...
Try this workaround. It does a simple build and serve whenever something
changes. It's not as good as a proper `nikola auto` because that nicely injects
some javascript that reloads the page.

```bash
find files posts | entr -r bash auto.sh
```

Consider `brew install entr` if on MacOS, similar things exist for other OS'es.
